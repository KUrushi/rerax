...
class TestTrainer:
    ...
    def test_train_step_updates_params_without_jit(self):
        ...
        # パラメータのコピーを取得
        initial_params = jax.tree_util.tree_map(lambda x: x.copy(), nnx.state(model, nnx.Param))
...
class TestTrainerFit:
    ...
    def test_fit_reduce_loss(self):
        ...
        class LinearModel(nnx.Module):
            def __init__(self, rngs: nnx.Rngs):
                self.linear = nnx.Linear(1, 1, kernel_init = nnx.initializers.zeros,rngs=rngs)
        ...
        assert isinstance(history, list) or isinstance(history, dict) or isinstance(history, nnx.MultiMetric)
